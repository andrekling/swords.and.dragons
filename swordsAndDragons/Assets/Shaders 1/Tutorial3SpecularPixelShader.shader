Shader "AndreTrainning/Tutorial3SpecularPixelShader" {
	Properties {
	...
	struct VertexOutput
	 ...
	 //float4 col :COLOR; //We dont need to pass color anymore since we will calculate in the fragment shader
	...
	o.posWorld = mul(_Object2World, v.vertex);// since we cant access o.pos from fragment shader
	...
	//Fragment program
		...
		//Vectors
	float3 normalDirection = i.normalDir; //Vector pointing from the normal of object out
	...
	float3 viewDirection = normalize(float3(float4(_WorldSpaceCameraPos.xyz,0.0) - i.posWorld.xyz));//Vector from camera pointing to object, Must be to objectSpace and not UnityMatrx_MVP
	...
	float3 lightFinal = diffReflection + specReflection + UNITY_LIGHTMODEL_AMBIENT;
		...
		return float4(lightFinal * _Color,1.0); //We return the light calculation * color
		}
