Shader "AndreTrainning/TestToon" {
	Properties
	{
		_Color("Color Tint",Color) = (1.0,1.0,1.0,1.0)
		//Added a flow map
		_FlowMap("Flow Map", 2D) = "White"{} //Flow Map texture used to rotate the diffuse
	...
	SubShader{
		...
		uniform float4 _FlowMap_ST;// ST - Super Transformations
		...
		float4 texF = tex2D(_FlowMap, i.tex.xy * _FlowMap_ST.xy + _FlowMap_ST.zw);
		float4 tex = tex2D(_MainTexture, i.tex.xy * _MainTexture_ST.xy * texF.xy + _MainTexture_ST.zw); // we declare texture and we pass the variable as a sampler 2d, and we unwrap it by passing the uv coordinates, multiplied by the ..._RT.xy and add the .zw this way the tiling works
