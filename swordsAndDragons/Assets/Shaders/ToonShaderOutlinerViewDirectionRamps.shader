		...
		struct SurfaceOutputAndre
{
    ...
    fixed Alpha;    // alpha for transparencies
		...
		//col.rgb = (s.Albedo * (((wetRamp * s.Alpha) * _WetnessValue)*_WetnessStrenght )+ _AddValue) * _LightColor0.rgb * ((difLight + 0.5) * sm5) * _Color * s.Albedo;
		col.rgb = tm5;
		//col.rgb = s.Alpha;
		...
		fixed4 ViewTex = tex2D(_ViewDirectionTexture, IN.uv_MainTex);
